...
const TileData* TileData::pointOverlaps(const std::vector<TileData>& tileData, int tileSize, signed char layer, int x, int y) {
...
const TileData* TileData::rectOverlaps(const std::vector<TileData>& tileData, int tileSize, signed char layer, int x1, int y1, int x2, int y2) {
	for (auto t = tileData.begin(); t != tileData.end() && t->layer >= layer; t++)
